class MockLoader
  ...
  OPERATING_SYSTEMS = { # rubocop:disable Style/MutableConstant
    ...
    windows_server_2016: { name: 'windows_server_2016_datacenter', family: 'windows', release: '10.0.14393',
                           arch: 'x86_64' },
  ...
  # Add a method to reset the shared platform
  ...
  def initialize(os = :ubuntu)
    self.class.reset_shared_platform
  ...
  def load_resource(res, *args)
    # If a platform override is passed, set @platform accordingly
    @platform = OPERATING_SYSTEMS[ args[0][:os] ] || @platform if args[0].is_a?(Hash) && args[0][:os]

    # initialize resource with backend and parameters
    @resource_class = Inspec::Resource.registry[res]
    raise ArgumentError, "No resource #{res}" unless @resource_class

    @resource = @resource_class.new(backend, res, *args)
  end
